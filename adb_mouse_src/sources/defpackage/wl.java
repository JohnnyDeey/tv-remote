package defpackage;

import java.math.BigInteger;
import java.security.SecureRandom;
import java.util.Collections;
import java.util.HashMap;
import java.util.Map;
import java.util.concurrent.atomic.AtomicReference;
import java.util.logging.Logger;

/* loaded from: classes.dex */
public abstract class wl {
    public static final vl a;
    public static final AtomicReference b;
    public static final AtomicReference c;

    static {
        Logger.getLogger(wl.class.getName());
        new tl("globalConfig");
        new tl("threadLocalConfig");
        new tl("defaultRandomConfig");
        new tl("constraints");
        ThreadLocal threadLocal = new ThreadLocal();
        Map synchronizedMap = Collections.synchronizedMap(new HashMap());
        a = new vl();
        Object obj = new Object();
        b = new AtomicReference();
        AtomicReference atomicReference = new AtomicReference();
        c = atomicReference;
        fn fnVar = new fn(new BigInteger("fca682ce8e12caba26efccf7110e526db078b05edecbcd1eb4a208f3ae1617ae01f35b91a47e6df63413c5e12ed0899bcd132acd50d99151bdc43ee737592e17", 16), new BigInteger("962eddcc369cba8ebb260ee6b6a126d9346e38c5", 16), new BigInteger("678471b27a9cf44ee91a49c5147db1a9aaf244f05a434d6486931d2d14271b9e35030b71fd73da179069b32e2935630e1c2062354d0da20a6c416e50be794ca4", 16), new gn(123, h10.a("b869c82b35d70e1b1ff91b28e37a62ecdc34409b")));
        fn fnVar2 = new fn(new BigInteger("e9e642599d355f37c97ffd3567120b8e25c9cd43e927b3a9670fbec5d890141922d2c3b3ad2480093799869d1e846aab49fab0ad26d2ce6a22219d470bce7d777d4a21fbe9c270b57f607002f3cef8393694cf45ee3688c11a8c56ab127a3daf", 16), new BigInteger("9cdbd84c9f1ac2f38d0f80f42ab952e7338bf511", 16), new BigInteger("30470ad5a005fb14ce2d9dcd87e38bc7d1b1c5facbaecbe95f190aa7a31d23c4dbbcbe06174544401a5b2c020965d8c2bd2171d3668445771f74ba084d2029d83c1c158547f3a9f1a2715be23d51ae4d3e5a1f6a7064f316933a346d3f529252", 16), new gn(263, h10.a("77d0f8c4dad15eb8c4f2f8d6726cefd96d5bb399")));
        fn fnVar3 = new fn(new BigInteger("", 16), new BigInteger("9760508f15230bccb292b982a2eb840bf0581cf5", 16), new BigInteger("", 16), new gn(92, h10.a("8d5155894229d5e689ee01e6018a237e2cae64cd")));
        fn fnVar4 = new fn(new BigInteger("", 16), new BigInteger("f8183668ba5fc5bb06b5981e6d8b795d30b8978d43ca0ec572e37e09939a9773", 16), new BigInteger("", 16), new gn(497, h10.a("b0b4417601b59cbc9d8ac8f935cadaec4f5fbb2f23785609ae466748d9b5a536")));
        fn[] fnVarArr = {fnVar, fnVar2, fnVar3, fnVar4};
        if (fn.class.isAssignableFrom(fnVarArr[0].getClass())) {
            Map map = (Map) threadLocal.get();
            if (map == null) {
                map = new HashMap();
                threadLocal.set(map);
            }
            map.put("dsaDefaultParams", fnVarArr);
            synchronizedMap.put("dsaDefaultParams", fnVarArr);
            ym[] ymVarArr = {c(fnVar), c(fnVar2), c(fnVar3), c(fnVar4)};
            if (ym.class.isAssignableFrom(ymVarArr[0].getClass())) {
                Map map2 = (Map) threadLocal.get();
                if (map2 == null) {
                    map2 = new HashMap();
                    threadLocal.set(map2);
                }
                map2.put("dhDefaultParams", ymVarArr);
                synchronizedMap.put("dhDefaultParams", ymVarArr);
                atomicReference.set(obj);
                atomicReference.get();
                return;
            }
            c.k("Bad property value passed");
            return;
        }
        c.k("Bad property value passed");
    }

    public static void a() {
        ((ul) c.get()).getClass();
    }

    public static SecureRandom b() {
        AtomicReference atomicReference;
        do {
            atomicReference = b;
            if (atomicReference.compareAndSet(null, a)) {
                break;
            }
        } while (atomicReference.get() == null);
        ThreadLocal threadLocal = ((vl) atomicReference.get()).a;
        if (threadLocal.get() == null) {
            threadLocal.set(new SecureRandom());
        }
        return (SecureRandom) threadLocal.get();
    }

    /* JADX WARN: Type inference failed for: r3v1, types: [ym, java.lang.Object] */
    public static ym c(fn fnVar) {
        int i;
        BigInteger bigInteger = fnVar.c;
        gn gnVar = fnVar.d;
        int bitLength = bigInteger.bitLength();
        if (bitLength > 1024) {
            if (bitLength <= 2048) {
                i = 224;
            } else if (bitLength <= 3072) {
                i = 256;
            } else if (bitLength <= 7680) {
                i = 384;
            } else {
                i = 512;
            }
        } else {
            i = 160;
        }
        BigInteger bigInteger2 = fnVar.a;
        BigInteger bigInteger3 = fnVar.b;
        f20.e(f20.e(gnVar.a));
        ?? obj = new Object();
        if (i > bigInteger.bitLength() && !qf0.b("org.bouncycastle.dh.allow_unsafe_p_value")) {
            c.k("unsafe p value so small specific l required");
            return null;
        }
        obj.a = bigInteger2;
        obj.b = bigInteger;
        obj.c = bigInteger3;
        return obj;
    }
}
